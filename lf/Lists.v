...
(* Alternate version, not sure if easier/harder to reason about *)
(*   | hd :: tl => (if (hd =? v) then nil else [hd) ++ (remove_all v tl) *)
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool := 
  match s1 with
  | nil => true
  | hd :: tl => if (member hd s2) then (included tl (remove_one hd s2)) else false
  end
.
...
Lemma eq_self : forall (n: nat), (n =? n) = true.
Proof.
induction n as [|n' H].
  - reflexivity.
...
Theorem add_inc_count : forall (b: bag) (v: nat), (count v b) + 1 = count v (add v b).
Proof.
  intros.
  destruct b eqn:E.
  - induction v as [|v' H].
  ...
  - simpl. rewrite eq_self. simpl. rewrite <- plus_1_r. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite H. simpl. rewrite app_assoc. reflexivity.
Qed.
...
Proof.
  ...
  induction l as [|n l' H].
  - reflexivity.
  - simpl. rewrite eq_self. rewrite <- H. reflexivity.
Qed.
...
Proof.
  intros.
  induction s as [|hd s' H]; reflexivity.
Qed.
...
Theorem involution_injective' : forall (f : natlist -> natlist),
...
Proof.
  intros.
  apply (involution_injective' rev rev_involutive' _ _). apply H.
Qed.
...
Proof.
  intros. 
  destruct x. simpl. rewrite NatList.eq_self. reflexivity.
Qed.
